...
public:
	response_helper(service &_service, const std::string &file_name, std::fstream &file);
...
void service::call_static_resource(const std::string &file_name)
{
	...
	if( it == request.headers().end() )
		response_helper(*this, file_name, file).default_transfer();

	else if( not it->second.empty() ) // Range: bytes=x-y, m-n, i-j ...
		response_helper(*this, file_name, file).range_transfer(it->second);
	else
		return_to_null(http::hs_bad_request);
...
response_helper::response_helper(service &_service, const std::string &file_name, std::fstream &file) :
	m_service(_service),
...
//		response.set_header("content-encoding", "gzip");
	...
	m_service.response.set_header("content-length", file_size);
	m_service.response.set_header("content-type",   mime_type);
	m_service.response.set_header("access-control-allow-origin", "*");

	asio::error_code error;
	if( file_size == 0 )
	{
		m_service.socket.write_some(asio::buffer(m_service.response.to_string()), error);
		return ;
	}
	...
	for(;;)
	{
		...
		m_service.socket.write_some(asio::buffer(fr_buf, size), error);
		if( size < buf_size )
			break;
	}
	delete[] fr_buf;
...
void response_helper::range_transfer(const std::string &range_str)
{
	...
	if( range_str.substr(0,6) != "bytes=" )
		return m_service.return_to_null(http::hs_range_not_satisfiable);
	...
	m_service.response.set_status(http::hs_partial_content);
	...
	if( range_list.size() == 1 )
	{
		...
		m_service.response.set_header("access-control-allow-origin", "*");
		m_service.response.set_header("accept-ranges", "bytes");

		m_service.response.set_header("content-type", mime_type);
		m_service.response.set_header("content-length", range_value.size);

		m_service.response.set_header
	...
	m_service.response.set_header("content-type", "multipart/byteranges; boundary=" + boundary);
	...
	m_service.response.set_header("content-length", content_length);
...
bool response_helper::range_parsing
...
(const std::string &boundary, const std::string &ct_line, std::list<range_value> &range_value_queue)
{
	...
	if( range_value_queue.size() == 1 )
	{
		...
		do {
			if( value.size <= buf_size )
			{
				...
				m_service.socket.write_some(asio::buffer(buf, s), error);
				delete[] buf;
				flag = false;
			}
			else
	...
	{
		m_service.socket.write_some(asio::buffer(
										  "--" + boundary + "\r\n" +
										  ct_line + "\r\n" +
										  value.cr_line + "\r\n" +
										  "\r\n"),
		...
		do {
			if( value.size <= buf_size )
			{
				...
				m_service.socket.write_some(asio::buffer(buf, s + 2), error);
				delete[] buf;
				flag = false;
			}
			else
	...
	m_service.socket.write_some(asio::buffer("--" + boundary + "--\r\n"), error);
}
